...
jQuery(document).ready(
	function () {
		...
		jQuery("input[type='range'][name^='alg_wc_pif_']").each((index, value) => {
			...
			let minVal = jQuery(value).attr('min') || 0;
			let maxVal = jQuery(value).attr('max') || 100;
			jQuery(value).before('<span class="alg_wc_pif_range_min">' + minVal + '</span>');
			jQuery(value).after('<span class="alg_wc_pif_range_bubble" style="float:right;" id="' + id + '_bubble">' + jQuery(value).val() + '</span>');
			jQuery(value).after('<span class="alg_wc_pif_range_max">' + maxVal + '</span>');
