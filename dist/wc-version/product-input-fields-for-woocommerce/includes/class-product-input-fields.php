...
final class Product_Input_Fields {
	...
	public function init() {
		if ( is_admin() ) {
			add_filter( 'plugin_action_links_' . PIF_PRO_PLUGIN_BASENAME, array( $this, 'action_links' ) );
		}
	...
	public static function activate_plugin() {
		...
		if ( ! get_option( 'pif_general_settings', false ) ) {
			...
			update_option( 'alg_wc_pif_version', PIF_PRO_VERSION );
		}
	...
	public static function include_file( $file, $is_plugin_include_file = true ) {
		$file = $is_plugin_include_file ? PIF_PRO_PLUGIN_DIR_PATH . '/includes/' . $file : $file;
	...
	public static function define_constants() {
		...
		self::define( 'PIF_PRO_ITEM_NAME', 'Product Input Fields for WooCommerce' );
		self::define( 'PIF_SLUG', self::$slug );
		self::define( 'PIF_PLUGIN_SLUG', self::$plugin_slug );
		self::define( 'PIF_PRO_VERSION', self::$plugin_version );
		self::define( 'PIF_PRO_PLUGIN_BASENAME', plugin_basename( PIF_FILE ) );
		self::define( 'PIF_PRO_PLUGIN_DIR_PATH', plugin_dir_path( PIF_FILE ) );
		self::define( 'PIF_PRO_PLUGIN_URL', plugins_url( '/', PIF_FILE ) );
        self::define( 'PIF_PRO_STORE_URL', 'https://www.tychesoftwares.com/' );
        self::define( 'PIF_PRO_ID', 'alg_wc_pif_pro' );
	...
	public function pif_include_updating_data() {
		if ( ! class_exists( 'PIF_Update' ) ) {
			require_once PIF_PRO_PLUGIN_DIR_PATH . 'includes/class-pif-update.php';
		}
